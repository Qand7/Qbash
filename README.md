...
## qfind
Utility I made because I found the ```find``` command too long to always remember

# Usage

## Compatibility
I'm not sure whether this works on all Distros because I only tried Arch Linux and raspian(Debian) but make sure to replace  
```$GOALDIR = "usr/bin/"``` in ```install.sh``` with whichever directory your shell commands are saved in (Working on automatic recognition)
